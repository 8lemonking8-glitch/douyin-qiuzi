...
function render(state) {
  ...
  $('roomConnectionDetail').textContent = state.directDycastDetail || state.dycastError || '尚未连接';
...
$('connectRoomBtn').onclick = async () => { const button = $('connectRoomBtn'); button.disabled = true; $('roomNumber').disabled = true; try { await directProvider.start($('roomNumber').value); } catch (error) { if (!engine.snapshot().dycastError) engine.setDirectDycastStatus({ error: error.message, detail: `连接失败：${error.message}` }); } finally { button.disabled = false; $('roomNumber').disabled = false; } }; $('disconnectRoomBtn').onclick = () => directProvider.stop();
