...
$('connectRoomBtn').onclick = async () => { $('roomConnectionDetail').textContent = '正在校验网页房间号并连接…'; try { await directProvider.start($('roomNumber').value); } catch (error) { engine.setDirectDycastStatus({ error: error.message }); } }; $('disconnectRoomBtn').onclick = () => directProvider.stop();
