...
/**
 * Dycast Desktop connects with the web_rid from live.douyin.com, not the
 * 18/19-digit internal room_id contained in mobile share/reflow links.
 */
export function parseDycastRoomInput(value) {
  ...
  if (/https?:\/\/v\.douyin\.com\//i.test(input) || /webcast\.amemv\.com\/douyin\/webcast\/reflow\//i.test(input)) {
    throw new Error('手机“复制链接”得到的短链包含内部 room_id，Dycast 不能用它连接。请在电脑浏览器打开抖音直播页，复制地址栏中的 https://live.douyin.com/数字。');
  }
  ...
  if (/^\d{16,20}$/.test(input)) {
    throw new Error('这是内部 room_id，不是 Dycast 所需的网页房间号。请复制 live.douyin.com/数字 地址。');
  }
