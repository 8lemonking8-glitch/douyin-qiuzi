...
export class QuizEngine {
  ...
  initialState() {
    return { idx: 0, phase: 'idle', remaining: this.roundSeconds, stats: { A: 0, B: 0, C: 0, D: 0 }, answers: {}, players: {}, recent: [], winner: null, dycastConnected: 0, directDycastConnected: 0, directDycastConnecting: false, directDycastRoom: null, directDycastDetail: '尚未连接', externalDycastConnected: 0, dycastLastMessageAt: null, dycastError: null };
  }
  ...
  snapshot() {
    ...
    return {
      ...
      dycastConnected: this.state.dycastConnected, directDycastConnected: this.state.directDycastConnected, directDycastConnecting: this.state.directDycastConnecting, directDycastRoom: this.state.directDycastRoom, directDycastDetail: this.state.directDycastDetail, dycastLastMessageAt: this.state.dycastLastMessageAt, dycastError: this.state.dycastError
    };
  }
  ...
  setDirectDycastStatus({ connected = 0, error = null, connecting = false, roomNumber = null, detail = null } = {}) { this.state.directDycastConnected = Number(connected) || 0; this.state.directDycastConnecting = Boolean(connecting); this.state.directDycastRoom = roomNumber; this.state.directDycastDetail = detail || (connected ? `已连接房间 ${roomNumber || ''}` : connecting ? `正在连接房间 ${roomNumber || ''}…` : error || '尚未连接'); this.state.dycastConnected = this.state.directDycastConnected || this.state.externalDycastConnected; this.state.dycastError = error; this.notify(); }
  startRound() {
